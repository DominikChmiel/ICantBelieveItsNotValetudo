...
class MqttClient {
    ...
    constructor(options) {
        ...
        this.topics = {
            map: this.topicPrefix + "/" + this.identifier + "/MapData/map_extern",
            position_x: this.topicPrefix + "/" + this.identifier + "/RobotPos/x",
            position_y: this.topicPrefix + "/" + this.identifier + "/RobotPos/y",
            homeassistant_autoconf_map: this.autoconfPrefix + "/camera/" + this.identifier + "/" + this.topicPrefix + "_" + this.identifier + "_map/config",
            homeassistant_autoconf_px: this.autoconfPrefix + "/sensor/" + this.identifier + "/" + this.topicPrefix + "_" + this.identifier + "_pos_x/config",
            homeassistant_autoconf_py: this.autoconfPrefix + "/sensor/" + this.identifier + "/" + this.topicPrefix + "_" + this.identifier + "_pos_y/config"
        };
        ...
        this.autoconf_payloads = {
            ...
            position_x: {
                name: this.identifier + " Position (x)",
                unique_id: this.identifier + "_position_x",
                device: { name: this.identifier, identifiers: [this.identifier] },
                state_topic: this.topics.position_x
            },
            position_y: {
                name: this.identifier + " Position (y)",
                unique_id: this.identifier + "_position_y",
                device: { name: this.identifier, identifiers: [this.identifier] },
                state_topic: this.topics.position_y
            }
        };
        ...
        setInterval(() => {
            ...
            let pointsToCanvas = (coords) => coords.map(d => Math.floor(d / this.mapDrawer.mapData.pixelSize));
            const robotPositionEntity = this.mapDrawer.mapData.entities.find(e => e.type === "robot_position");
            if (robotPositionEntity) {
                const robotPosition = this.mapDrawer.translatePoints(pointsToCanvas(robotPositionEntity.points));
                Logger.warn(JSON.stringify(robotPositionEntity));
                Logger.warn(JSON.stringify(robotPosition));
                this.client.publish(this.topics.position_x, "" + robotPosition[0], { retain: true });
                this.client.publish(this.topics.position_y, "" + robotPosition[1], { retain: true });
            }
            ...
            this.client.on("connect", () => {
                ...
                this.client.subscribe([
                    ...
                    if (!err) {
                        ...
                        this.client.publish(this.topics.homeassistant_autoconf_px, JSON.stringify(this.autoconf_payloads.position_x), {
                            retain: true
                        });
                        this.client.publish(this.topics.homeassistant_autoconf_py, JSON.stringify(this.autoconf_payloads.position_y), {
                            retain: true
                        });
